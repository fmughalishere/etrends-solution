    ...
    <main className="bg-[#0a0a0a] min-h-screen">
      ...
      <section className="py-24 bg-[#0d0d0d]">
        <div className="max-w-[1400px] mx-auto px-6">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 bg-white/5">
            {SERVICES_DATA.map((service) => (
              <Link
                key={service.id}
                href={`/services/${service.id}`}
                className="group relative h-[320px] overflow-hidden flex flex-col justify-end p-10"
              >
                <Image
                  src={service.image}
                  alt={service.title}
                ...
                <div className="absolute inset-0 bg-[#00aef0]/80 opacity-0 group-hover:opacity-100 transition-opacity duration-500" />
                <div className="relative z-10">
                  <div className="flex items-center gap-4 mb-4">
                    <div className="w-8 h-[1px] bg-white group-hover:bg-black transition-all" />

                    <h3 className="text-lg md:text-xl font-bold text-white uppercase tracking-widest group-hover:text-black transition-colors">
                      {service.title}
                    </h3>
                  </div>

                  <p className="text-gray-300 text-sm opacity-0 group-hover:opacity-100 group-hover:text-black/80 transition-all duration-500 transform translate-y-4 group-hover:translate-y-0">
                    {service.desc}
                  </p>
